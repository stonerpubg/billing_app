...
const emptyItem = () => ({
  ...
  size: '',
  weight: '',
  // 'size'  → amount = size × qty × rate (default; when size is blank it collapses to qty × rate)
  // 'weight' → amount = weight × rate
  price_by: 'size',
  quantity: 1,
  rate: 0,
  gst_rate: 18,
...
// Compute the line amount for an item given its pricing mode.
// price_by='weight' → weight × qty × rate  (per-unit weight × pieces × ₹/kg rate)
// price_by='size'   → size × qty × rate    (default; blank size collapses to qty × rate)
...
export default function QuotationEditor() {
  ...
  const [form, setForm] = useState({
    ...
    notes: '',
    terms: '',
    gst_mode: 'per_line', // 'per_line' or 'flat_on_total'
    flat_gst_rate: 18,
    items: [emptyItem()],
  });
  ...
  // Merge parsed voice data into the form. Customer info is preloaded into the
  // "Add customer" modal so the user can save + attach it in one confirm.
  const applyVoiceFill = (parsed) => {
    if (!parsed) return;
    // Subject + GST mode (spoken with "flat gst 18" or "per product gst")
    setForm((f) => {
      const patch = {};
      if (parsed.subject) patch.subject = parsed.subject;
      if (parsed.gstMode) patch.gst_mode = parsed.gstMode;
      if (parsed.flatGstRate != null && parsed.gstMode === 'flat_on_total') patch.flat_gst_rate = parsed.flatGstRate;
      return Object.keys(patch).length ? { ...f, ...patch } : f;
    });
    // Items — replace the initial blank row (single empty row on a fresh form)
    if (parsed.items && parsed.items.length > 0) {
      setForm((f) => {
        const isInitialBlank = f.items.length === 1 && !f.items[0].name && !f.items[0].rate;
        const merged = parsed.items.map((it) => ({
          ...
          gst_rate: it.gst_rate != null ? it.gst_rate : 18,
        }));
        return { ...f, items: isInitialBlank ? merged : [...f.items, ...merged] };
      });
    }
    // Customer — if voice provided one and no existing customer is picked,
    // prefill + open the "Add customer" modal so the user confirms & saves.
    if (parsed.customer && parsed.customer.name && !form.customer_id) {
      setNewCustomer((c) => ({
        ...c,
        name: parsed.customer.name || c.name,
        phone: parsed.customer.phone || c.phone,
        gstin: parsed.customer.gstin || c.gstin,
        city: parsed.customer.city || c.city,
        state: parsed.customer.state || c.state,
        address: parsed.customer.address || c.address,
        pincode: parsed.customer.pincode || c.pincode,
      }));
      setShowCustomer(true);
    }
    toast.success(
      `Filled ${parsed.items.length} item${parsed.items.length === 1 ? '' : 's'}` +
      (parsed.customer?.name ? ' + customer' : '') +
  ...
  useEffect(() => {
    (async () => {
      ...
      if (isEditing) {
        const q = await window.api.quotations.get(Number(id));
        if (q) {
          setForm({
            ...
            flat_gst_rate: q.flat_gst_rate != null ? q.flat_gst_rate : 18,
        ...
        const next = await window.api.quotations.nextNumber();
  ...
  const totals = useMemo(() => {
    let subtotal = 0;
    let gst = 0;
    // Per-rate breakdown for the "GST %" summary above Grand Total in the PDF
    const gstBreakdown = {};
  ...
  const validate = () => {
    if (!form.customer_id) return 'Please select or add a customer (Quote To).';
    if (!form.quote_date) return 'Quote date is required.';
  ...
  const save = async () => {
    ...
    const payload = { ...form };
    if (isEditing) {
      await window.api.quotations.update({ ...payload, id: Number(id) });
      toast.success('Quotation updated');
      nav(`/quotations/${id}`);
    } else {
      const created = await window.api.quotations.create(payload);
      toast.success(`Quotation ${created.quote_number} created`);
      nav(`/quotations/${created.id}`);
    }
  };
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle={
          isEditing
            ? `Quote No. ${form.quote_number}`
            : `Next number will be: ${previewNumber || '…'}`
        }
        right={
          <>
            <button
              ...
              title="Fill this quotation by speaking"
            >
              🎤 Voice fill
            </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
        <div className="card lg:col-span-2">
          <div className="card-header">
            ...
            <button className="btn-ghost text-xs" onClick={() => setShowCustomer(true)}>
              + Add customer
            </button>
          </div>
          <div className="card-body space-y-3">
            ...
            <div>
              ...
              <input
                className="input"
                placeholder="e.g. Roofing & plumbing works at site X"
                value={form.subject}
      ...
      <div className="card">
          ...
          <table className="w-full min-w-[1520px] table-fixed">
            ...
            <tbody>
              {form.items.map((it, idx) => {
                ...
                const amount = computeLineAmount(it);
                const perLineGst = form.gst_mode === 'per_line';
                const gstAmt = perLineGst ? (amount * (Number(it.gst_rate) || 0)) / 100 : 0;
                ...
                return (
                  <tr key={idx} className="align-top">
                    <td className="td">
                      ...
                      <input
                        className="input py-2 text-sm mt-1 w-full"
                    ...
                    <td className="td">
                      <textarea
                        className="input py-2 text-sm min-h-[72px] w-full"
                    ...
                    <td className="td">
                      ...
                      <button
                        ...
                        title={byWeight ? 'Amount = weight × qty × rate. Click to switch to Size × Qty × Rate.' : 'Amount = size × qty × rate. Click to bill by Weight × Qty × Rate.'}
                      >
                    ...
                    <td className="td">
                      <input
                        ...
                        step="0.01"
                        className={
                          'input py-2 text-sm text-right w-full ' +
                          (!perLineGst ? 'opacity-40 cursor-not-allowed' : '')
                        }
                        disabled={!perLineGst}
                        title={!perLineGst ? 'GST is computed on the whole subtotal (flat mode)' : ''}
                        value={it.gst_rate}
                        onChange={(e) => updateItem(idx, { gst_rate: e.target.value })}
                      />
                    </td>
                    <td className={'td text-right text-sm ' + (!perLineGst ? 'text-slate-400' : '')}>
                      {perLineGst ? money(gstAmt) : '—'}
                    </td>
        ...
        <div className="p-5 grid grid-cols-1 lg:grid-cols-3 gap-4 border-t border-slate-200">
          <div className="lg:col-span-2 space-y-3">
            <div>
              <label className="label">Notes (visible on PDF)</label>
              <textarea
          ...
          <div className="bg-slate-50 rounded-md p-4 h-fit border border-slate-200 space-y-3">
            ...
            <div>
              ...
              <div className="flex gap-1 p-1 bg-white rounded-md border border-slate-200">
                <button
                  type="button"
                  onClick={() => setForm((f) => ({ ...f, gst_mode: 'per_line' }))}
                  className={
                    'flex-1 text-xs py-1.5 rounded font-medium transition ' +
                    (form.gst_mode === 'per_line'
                ...
                <button
                  type="button"
                  onClick={() => setForm((f) => ({ ...f, gst_mode: 'flat_on_total' }))}
                  className={
                    'flex-1 text-xs py-1.5 rounded font-medium transition ' +
              ...
              {form.gst_mode === 'flat_on_total' && (
                <div className="mt-2 flex items-center gap-2">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setForm((f) => ({ ...f, flat_gst_rate: e.target.value }))
                    }
                  />
                </div>
              )}
      ...
      <Modal open={showCustomer} title="Add customer" onClose={() => setShowCustomer(false)} size="lg">
        ...
        <div className="p-5 border-t border-slate-200 flex justify-end gap-2">
          <button className="btn-secondary" onClick={() => setShowCustomer(false)}>Cancel</button>
